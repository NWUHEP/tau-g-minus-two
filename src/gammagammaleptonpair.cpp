					...
					double &px2, double &py2, double &pz2)
{
    ...
    double anglepp1=0.,anglepp2=0.,pp1=0.,pp2=0.,E1CM=0.,E2CM=0.;
    double pt=0.;
    ...
    double gammaCM = E/(E1CM+E2CM);
    double E1 = E1CM*gammaCM;
    double E2 = E2CM*gammaCM;

    //when EPA is violated:
    //Massless Photons -->
    //E1=Mag(P1), E2=Mag(P2), P1+P2=PLeptons --> only an angle of the addition plane is ambiguous
    //define random vector cross P_Lepton direction as t plane vector
    if(pp1>E1 || pp2>E2) {
      double plep = sqrt(pt*pt+pz*pz);
      double cos2 = 1/(2*E2*plep)*(plep*plep + E2*E2 - E1*E1);
      double cos1 = 1/(2*E1*plep)*(plep*plep + E1*E1 - E2*E2);
      double sin1 = sqrt(1-cos1*cos1);
      double sin2 = -sqrt(1-cos2*cos2);
      double plhat[] = {px/plep, py/plep, pz/plep};
      //random vector, between -0.5 and 0.5
      double r[] = {_randy.Rndom()-0.5, _randy.Rndom()-0.5, _randy.Rndom()-0.5};
      //vector perpendicular to lepton momentum
      //used random r cross lepton momentum to get in random direction
      double t[] = {r[1]*plhat[2]-r[2]*plhat[1],
		    r[2]*plhat[0]-r[0]*plhat[2],
		    r[0]*plhat[1]-r[1]*plhat[0]};
    
      double tmag = sqrt(t[0]*t[0]+t[1]*t[1]+t[2]*t[2]);
      t[0] = t[0]/tmag; t[1]=t[1]/tmag; t[2]=t[2]/tmag;

      double photon1[] = {E1*(plhat[0]*cos1+t[0]*sin1),E1*(plhat[1]*cos1+t[1]*sin1),E1*(plhat[2]*cos1+t[2]*sin1)};
      double photon2[] = {E2*(plhat[0]*cos2+t[0]*sin2),E2*(plhat[1]*cos2+t[1]*sin2),E2*(plhat[2]*cos2+t[2]*sin2)};
      if(photon2[2] > 0) {
      px1 = photon1[0];
      py1 = photon1[1];
      pz1 = photon1[2];
      ...
      px1 = pp1*cos(2.*starlightConstants::pi*anglepp1);
      px2 = pp2*cos(2.*starlightConstants::pi*anglepp2);
      py1 = pp1*sin(2.*starlightConstants::pi*anglepp1);
      py2 = pp2*sin(2.*starlightConstants::pi*anglepp2);
      pz1 = sqrt(E1*E1 - pp1*pp1);
      pz2 = sqrt(E2*E2 - pp2*pp2);
      //determine which should have which sign
      double signP = pz/abs(pz); 
      if(abs(pz)+0.01 < pz1+pz2) {
	if(pz1>pz2){
	  pz1*=signP;
	  pz2*=-1*signP;
	}else{
...
{//returns the vector with the decay particles inside.
    ...
    double px2=0.,px1=0.,py2=0.,py1=0.,pz2=0.,pz1=0.,
      gamma1px=0.,gamma1py=0.,gamma1pz=0.,
      gamma2px=0.,gamma2py=0.,gamma2pz=0.;
    ...
    pairMomentum(comenergy,rapidity,pairE,pairmomx,pairmomy,pairmomz,
		 gamma1px, gamma1py, gamma1pz,
		 gamma2px, gamma2py, gamma2pz);
    
    twoBodyDecay(ipid,comenergy,pairmomx,pairmomy,pairmomz,px1,py1,pz1,px2,py2,pz2,iFbadevent);
    double betax= -(pairmomx)/pairE;
    double betay= -(pairmomy)/pairE;
    double betaz= -(pairmomz)/pairE;
    double Egamma1 = sqrt(gamma1px*gamma1px+gamma1py*gamma1py+gamma1pz*gamma1pz);
    double Egamma2 = sqrt(gamma2px*gamma2px+gamma2py*gamma2py+gamma2pz*gamma2pz);

    transform(betax,betay,betaz,Egamma1,gamma1px,gamma1py,gamma1pz,iFbadevent);
    transform(betax,betay,betaz,Egamma2,gamma2px,gamma2py,gamma2pz,iFbadevent);
	...
	beam beam1=_bbs.beam1();
	beam beam2=_bbs.beam2();
	int a1 = beam1.A();
	int z1 = beam1.Z();
	int a2 = beam2.A();
	int z2 = beam2.Z();
	double lorentz1 = beam1._beamLorentzGamma;
	double lorentz2 = beam2._beamLorentzGamma;
	...
	if(a1==1 && z1==1 &&
	   a2==1 && z2==1) {
	  //define beam 1 moving in +z direction
	  leptonpair.ppx1[0]=0;
	  leptonpair.ppy1[0]=0;
	  leptonpair.ppz1[0]=sqrt(1-1/lorentz1*1/lorentz1);
	  leptonpair.ppx2[0]=0;
	  leptonpair.ppy2[0]=0;
	  leptonpair.ppz2[0]=-sqrt(1-1/lorentz2*1/lorentz2);
	  
	  leptonpair.px1[0]=gamma1px;
	  leptonpair.py1[0]=gamma1py;
	  leptonpair.pz1[0]=gamma1pz;
	  leptonpair.px2[0]=gamma2px;
	  leptonpair.py2[0]=gamma2py;
	  leptonpair.pz2[0]=gamma2pz;
	}
...
//returns the vector with the decay particles inside.
   ...
   do{ 
     ...
     _nmbAttempts++;
     twoBodyDecay(ipid,comenergy,pairmomx,pairmomy,pairmomz,px1,py1,pz1,px2,py2,pz2,iFbadevent);
     //     double Ecm = sqrt(comenergy*comenergy+pairmomx*pairmomx
     //		       +pairmomy*pairmomy+pairmomz*pairmomz);
     
     //double betax= -(pairmomx)/Ecm;
     //double betay= -(pairmomy)/Ecm;
     // double betaz= -(pairmomz)/Ecm;
     //double Egamma1 = sqrt(gamma1px*gamma1px+gamma1py*gamma1py+gamma1pz*gamma1pz);
     //double Egamma2 = sqrt(gamma2px*gamma2px+gamma2py*gamma2py+gamma2pz*gamma2pz);
   ...
   if (iFbadevent==0){
     ...
     double gamma1E = sqrt(gamma1px*gamma1px + gamma1py*gamma1py + gamma1pz*gamma1pz); 
     double gamma2E = sqrt(gamma2px*gamma2px + gamma2py*gamma2py + gamma2pz*gamma2pz); 
	...
	a2==1 && z2==1) {
       ...
       double pmass = starlightConstants::protonMass;
       //define beam 1 moving in +z direction
       double ppz1=lorentz1*pmass*sqrt(1-1/lorentz1*1/lorentz1);
       double ppz2=-lorentz2*pmass*sqrt(1-1/lorentz2*1/lorentz2);
       double ppe1=lorentz1*pmass;
       double ppe2=lorentz2*pmass;
       starlightParticle particle1(0.0,0.0,ppz1,ppe1, starlightConstants::UNKNOWN, 2212, 0);
       particle1.setStatus(0);
       event.addParticle(particle1);

       starlightParticle particle2(0.0,0.0,ppz2,ppe2, starlightConstants::UNKNOWN, 2212, 0);
       particle2.setStatus(0);
       event.addParticle(particle2);

       starlightParticle particle3(gamma1px,gamma1py,gamma1pz,gamma1E, starlightConstants::UNKNOWN, 22, 0);
       particle3.setStatus(0);
       event.addParticle(particle3);

       starlightParticle particle4(gamma2px,gamma2py,gamma2pz,gamma2E, starlightConstants::UNKNOWN, 22, 0);
       event.addParticle(particle4);
       particle4.setStatus(0);

       starlightParticle particle5(-gamma1px,-gamma1py,ppz1-gamma1pz,ppe1-gamma1E, starlightConstants::UNKNOWN, 2212, 0);
       event.addParticle(particle5);
       particle5.setStatus(0);

       starlightParticle particle6(-gamma2px,-gamma2py,ppz2-gamma2pz,ppe2-gamma2E, starlightConstants::UNKNOWN, 2212, 0);
       particle6.setStatus(0);
       event.addParticle(particle6);
     ...
     starlightParticle particle7(px1, py1, pz1, E1, starlightConstants::UNKNOWN, -q1*ipid, q1);
     event.addParticle(particle7);
     
     starlightParticle particle8(px2, py2, pz2, E2, starlightConstants::UNKNOWN, -q2*ipid, q2);
     event.addParticle(particle8);
