...
starlight::init()
{
	...
	// Do some sanity checks of the input parameters here.
	  ...
	  printErrr << endl << "A must be >= Z; A beam1 = "<<_inputParameters->beam1A()<<", Z beam1 = "<<_inputParameters->beam1Z()<<". Terminating."<<endl ;
	  return false;
	}
        if( _inputParameters->beam2Z() > _inputParameters->beam2A() ){
	  printErrr << endl << "A must be >= Z; A beam2 = "<<_inputParameters->beam2A()<<", Z beam2 = "<<_inputParameters->beam2Z()<<". Terminating."<<endl ;
	  return false;
	}
	if( _inputParameters->interactionType() == PHOTONPOMERONINCOHERENT && _inputParameters->beam1A() == 1 &&
	    _inputParameters->beam1Z() == 1 && _inputParameters->beam2A() == 1 && _inputParameters->beam2Z() ){
          printErrr << endl << " Do not use PROD_MODE = 4 for pp collisions. Use PROD_MODE = 2 or 3 instead. Terminating."<<endl;
	  return false; 
	}
...
{
	if (!_isInitialised) {
		printErrr << "trying to generate event but Starlight is not initialised. aborting." << endl;
		exit(-1);
	}
